...
def main():
    cap = cv2.VideoCapture(Config.VIDEO_SAMPLE)
    video_info = {
    ...
    if Config.PEDESTRIAN_DETECTION:
    ...
    with open(Config.PL_LOCATIONS, 'r') as parking_lots:
        parking_data = yaml.load(parking_lots)
        ...
        if Config.PARKING_OVERLAY:
            for ind, park in enumerate(parking_data):
                ...
                cv2.putText(frame_out, str(park['id']), (centroid[0] - 1, centroid[1] + 1), cv2.FONT_HERSHEY_SIMPLEX,
                            0.5,
                            (255, 255, 255), 1, cv2.LINE_AA)
                cv2.putText(frame_out, str(park['id']), centroid, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1,
                            cv2.LINE_AA)
